...
- (void)awakeFromNib
{
	...
	if ( avail ) {
		...
		[ simple addObject:[ NSString stringWithFormat:@"Default Voice (%@)", [ voices[0] pathExtension ] ] ] ;
		for ( i = 1; i < size; i++ ) {
			[ simple addObject:[ voices[i] pathExtension ] ] ;
		}
...
- (Boolean)browseForField:(NSTextField*)textField into:(NSString**)filename isFolder:(Boolean)isFolder
{
	...
	result = [ panel runModalForTypes:@[@"scpt"] ] ;
...
- (void)setMatrix:(NSMatrix*)matrix fromKey:(NSString*)key
{
	...
	if ( count > 4 ) count = 4 ;
	for ( i = 0; i < count; i++ ) {
		[ [ matrix cellAtRow:i column:0 ] setStringValue:array[i] ] ;
	}
}
...
- (Boolean)updatePreferences
{
	...
	if ( [ self hasKey:kEnableNetAudio ] ) {
		if ( [ self intValueForKey:kEnableNetAudio ] != 0 ) ip = @([ application localHostIP ]) ;
	}
	...
	//  v0.89  MacroScripts
	NSArray *scriptArray = prefs[kMacroScripts] ;
	for ( i = 0; i < 6; i++ ) {
