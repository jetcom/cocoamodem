...
{
	if ( router == nil ) return [ NSArray array ] ;
...
- (void)useDigitalModeOnlyForFSK:(Boolean)state
{
	...
	n = [ keyers count ] ;
	for ( i = 0; i < n; i++ ) {
		[ (MicroKeyer*)[ keyers objectAtIndex:i ] useDigitalModeForFSK:state ] ;
	}
}
...
- (void)terminate:(Config*)config
{
	...
	if ( router ) {
		if ( [ config quitWithAutoRouting ] == YES ) {
			...
			n = [ keyers count ] ;
			for ( i = 0; i < n; i++ ) {
				[ (MicroKeyer*)[ keyers objectAtIndex:i ] setKeyerMode:kMicrohamAutoRouting ] ;
			}
		}
